...
export const SearchResultsScreen = ({navigation}) => {
    ...
    const filteredRooms = useContext(ClientContext).fetchedAllRooms.filter(room => {
    ...
    return (
        <View style={styles.container}>
            {filteredRooms.map((room, key) => {
                return (
                  <TouchableOpacity key={key} onPress={() => goToRoomInfo(room._id)}>
                    <View  style={styles.roomCardWrapper}>
                        <Image style={styles.image} source={{uri : config.API_URL + '/static/' + room.image}}/>
